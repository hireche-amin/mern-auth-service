...
const ResetPassword = () => {
 ...
 console.error('An error has occured in the otp submit handler',error)
    }
  }
  const newPasswordHandler = async (e) => {
    try{
      e.preventDefault();
      const response = await axios.post(backendUrl + '/auth/api/password-reset',{email,otp,newPassword})
      if(response.data.success){
